...
def extract_standings_data(url):
  ...
  for row in filtered_rows:
    ...
    df['Division'] = None
    df.loc[:7, 'Division'] = divisions[0][:-9].lower()
    df.loc[8:15, 'Division'] = divisions[1][:-9].lower()
    df.loc[16:23, 'Division'] = divisions[2][:-9].lower()
    df.loc[24:31, 'Division'] = divisions[3][:-9].lower()
